package research.lambda.effect.io.interpreter;

import com.jnape.palatable.lambda.adt.Unit;
import research.lambda.effect.io.IO;
import research.lambda.effect.io.Interpreter;
import research.lambda.effect.io.fiber2.old.FiberResult;
import research.lambda.effect.io.interpreter.TailExpr.Recur;
import research.lambda.effect.io.interpreter.TailExpr.Return;
...
import static research.lambda.effect.io.IO.io;
...
public final class RunSyncFiber<A> implements Interpreter<A, TailExpr<IO<A>, Unit>> {
    ...
    public TailExpr<IO<A>, Unit> interpret(A a) {
        terminate.accept(FiberResult.success(a));
    ...
    public <Z> TailExpr<IO<A>, Unit> interpret(
        ...
        return new Recur<>(ioZ.bind(z -> ioF.bind(f -> IO.io(f.apply(z)))));
    }
    ...
    private record Phi<Z, A>(
        ...
        public TailExpr<IO<A>, Unit> interpret(
            ...
            k.accept((Consumer<? super FiberResult<Z>>) resZ -> {
                if (resZ instanceof FiberResult.Failure<Z> failureZ) {
                    terminate.accept(failureZ.contort());
                } else if (resZ instanceof FiberResult.Cancelled<Z> cancelled) {
                    terminate.accept(cancelled.contort());
                } else {
                    recur.accept(f.apply(((FiberResult.Success<Z>) resZ).result()));
                }
            });
            return new Return<>(UNIT);
        }
